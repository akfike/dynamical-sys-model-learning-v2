...
def simulate_system_internal_dynamics(A, dt, num_time_steps, initial_state, noise_std=0.1):
    """
    ...
    for _ in range(1, num_time_steps):
        noise = np.random.normal(0, noise_std, size=x.shape)  # Generate Gaussian noise
        x = Ad @ x + noise  # Add noise to the state update
        states.append(x)
...
csv_file_path = 'data/internal_dynamics_data_with_noise.csv'
...
test_csv_file_path = 'data/test_internal_dynamics_data_with_noise.csv'
